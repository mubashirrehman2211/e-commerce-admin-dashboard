...
<template>
  <form @submit.prevent="productStore.addProduct(product, router)"
        class="md:w-[500px] border border-gray-700 my-10 w-full m-auto p-4 shadow-lg rounded-lg space-y-10">
    <h2 class="text-2xl text-color font-semibold text-center">Add New Product</h2>
      ...
      <div>
        <div class="relative">
          ...
          >
            ...
            <option :value="category" v-for="category in productTypes" :key="category">{{ category }}</option>
          </select>
        </div>
      </div>
      ...
      <div class="grow">
        <div class="relative">
          ...
          <input
              ...
              class="w-full pl-10 pr-3 py-2 border-b-2 border-gray-700 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
          />
          <img :src="product.image" alt="image-preview" v-if="product.image" class="w-full h-full object-contain border-2 rounded border-[var(--color-primary)]">
        </div>
      </div>
    </div>
